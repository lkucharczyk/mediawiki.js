...
export type OnlyOneOf<T extends Record<string, unknown>> = {
	[ mainkey in keyof T ]: {
		[ key in keyof T ]?: key extends mainkey ? T[mainkey] : never;
	} & Pick<T, mainkey>
}[keyof T];

type SubkeyOf<T extends Record<string, unknown>> = {
	[ mainkey in keyof T ]: keyof T[mainkey]
}[keyof T];

export type OnlyOneGroup<T extends Record<string, Record<string, unknown>>> = {
	[ mainkey in keyof T ]: {
...
export type ExtractVal<T, V, D = never> = V extends T ? V : V extends readonly T[] ? V[number] : D;
